...
function __loadStaticAppProps(lines){
    ...
    if (window.clientInformation.userAgent.indexOf('Windows NT 10.0') > -1) {
...
window.WLJSX.Ajax.Request = WLJSX.Class.create({
  ...
  request: function(url) {
    ...
    try {
      ...
      /*jshint eqeqeq:false*/
      this.body = this.method == 'post' ? (this.options.postBody || this.options.parameters) : null;
...
__WLSimpleDialog = function() {
  ...
  this.__callback = function(result) {
    ...
    } else if (WL.StaticAppProps.ENVIRONMENT === WL.Environment.WINDOWS8 || WL.StaticAppProps.ENVIRONMENT === WL.Environment.WINDOWS) {
      // empty
    } else {
...
__WLClient = function() {
	...
	function finalizeInit() {
		...
		if (WL.EnvProfile.isEnabled(WL.EPField.USES_CORDOVA) && WL.StaticAppProps.ENVIRONMENT!=WL.Environment.BLACKBERRY10 && WL.StaticAppProps.ENVIRONMENT!=WL.Environment.WINDOWS8
				&& WL.StaticAppProps.ENVIRONMENT != WL.Environment.WINDOWS ) {
      ...
      WL.Logger.warn('Initialization option \'analytics\' is deprecated. Use WL.Analytics.enable/disable to set analytics data capture.');
		...
		if (( typeof initOptions.enableFIPS === 'boolean') && (initOptions.enableFIPS)) {
        	...
        	window.WLJSX.Ajax.getTransport = function() {
                ...
                if (WL.Client.getEnvironment() == WL.Env.WINDOWS8 || WL.Client.getEnvironment() == WL.Environment.WINDOWS ) {
...
function WLResourceRequest(_url, _method, _timeout) {
  ...
  currentResourceRequest.scope = '';
  currentResourceRequest.isResourceProteced = false;
  ...
  function sendRequestAsync(contentString, attempt) {
    var dfd = WLJQ.Deferred();
    ...
    buildRequestUrl()
      .then(
        function (serverUrl) {
          ...
          __send(serverUrl, contentString, attempt).then(
            function (response) {
  ...
  function __send(serverUrl, contentString, attempt) {
    var dfd = WLJQ.Deferred();
    ...
    xhr.onreadystatechange = function () {
      if (this.readyState === 4) {
        ...
        if (this.status >= 200 && this.status <= 299) {
          ...
          if (typeof (WLAuthorizationManager) !== 'undefined' && WLAuthorizationManager.isAuthorizationRequired(transport) && attempt < maxRequestAttempts) {
    ...
    var processResponse = function (transport) {
      if (WLAuthorizationManager.__isForbidden(transport)) {
        currentResourceRequest.scope = WLAuthorizationManager.getResourceScope(transport);
        if (currentResourceRequest.scope !== null) {
          WLAuthorizationManager.__cacheScopeByResource(transport, currentResourceRequest.scope).always(resendRequest);
        }
      } else if (WLAuthorizationManager.__isInvalidToken(transport)) {
        WLAuthorizationManager.__removeTokenByScope(transport).always(resendRequest);
      } else {
        // We got 401 we need to obtain accessToken for empty scope
        currentResourceRequest.isResourceProteced = true;
        resendRequest();
      }
    };

    var resendRequest = function () {
      sendRequestAsync(contentString, ++attempt).then(
        function (response) {
          dfd.resolve(response);
        },
        function (error) {
    ...
    if (typeof (WLAuthorizationManager) !== 'undefined') {
      WLAuthorizationManager.__getCachedScopeByResource(xhr).always(
        function (cachedScope) {
          ...
          // If scope exist in the native cache or the resource is protected then invoke obtainAccessTokenWithScope
          if (currentResourceRequest.scope !== '' || currentResourceRequest.isResourceProteced) {
            WLAuthorizationManager.obtainAccessToken(currentResourceRequest.scope).then(
              function (accessToken) {
                // Send this request with accessToken.
              ...
              function (error) {
                ...
                WLAuthorizationManager.isResourceProteced = false;
                dfd.reject(error);
              }
            );
          }
          // Avoid requesting with accessToken to unprotected resource
          else {
            sendRequest();
          }
        });
    ...
    function sendRequest() {
      // Changing isResourceProtected flag to false - in case next time the resource won't be protected, so we don't want access it with default token. 
      WLAuthorizationManager.isResourceProteced = false;
...
window.WLAuthorizationManager = (function () {
  ...
  var clearAccessToken = function (accessToken) {
    ...
    cordova.exec(
      ...
      AUTHORIZATION_MANAGER_PLUGIN_NAME, 'clearAccessToken', [accessToken]
    );
  ...
  var obtainAccessToken = function (scope) {
    ...
    try {
      WLAuthorizationManager.__getCachedAccessToken(scope === EMPTY_SCOPE ? '' : scope)
        .then(
          function (accessToken) {
            if (typeof (accessToken) !== 'undefined' && accessToken !== null && typeof (accessToken.asAuthorizationRequestHeader) !== 'undefined') {
                ...
                .then(
                  function (clientId) {
                    if (typeof (clientId) !== 'undefined' && clientId !== null) {
                      ...
                      __invokePreAuthRequestWithScopeAndClientId(clientId, scope);
                    } else {
  ...
  function __invokePreAuthRequestWithScopeAndClientId(clientId, scope) {
    ...
    invokePreAuthorizationRequest(clientId, scope)
      .then(
        function (response) {
                  ...
                  WLAuthorizationManager.__invokeRegistrationRequest()
                    .then(
                      function (newClientId) {
                        ...
                        // resend the request
                        __invokePreAuthRequestWithScopeAndClientId(newClientId, scope);
                      },
                      function (error) {
  ...
  function processObtainAuthHeaderCallbacks(clientId, scope, response, isSuccess) {
    ...
    if (obtainAuthHeaderCallbacks.length > 0) {
      ...
      __invokePreAuthRequestWithScopeAndClientId(realClientId, obtainAuthHeaderCallbacks[0].scope);
    }
  ...
  var __invokeRegistrationRequest = function () {
    var dfd = WLJQ.Deferred();
    ...
    WLAuthorizationManager.__invokeInstanceRegistrationRequest()
      .then(
        function (data) {
  ...
  var __invokeInstanceRegistrationRequest = function () {
    var dfd = WLJQ.Deferred();
    cordova.exec(
      function (data) {
        dfd.resolve(data);
      ...
      AUTHORIZATION_MANAGER_PLUGIN_NAME, 'invokeInstanceRegistrationRequest', []
    );
    return dfd.promise();
  };
  ...
  function invokePreAuthorizationRequest(clientId, scope) {
    var dfd = WLJQ.Deferred();
    var request = null;
  ...
  var __removeTokenByScope = function (resource) {
    var dfd = WLJQ.Deferred();
    cordova.exec(
      function () {
        dfd.resolve();
      ...
      AUTHORIZATION_MANAGER_PLUGIN_NAME, 'removeTokenByScope', [resource]
    );
    return dfd.promise();
  };
  ...
  var isAuthorizationRequired = function (transport) {
    ...
    var responseAuthenticationHeader = transport.getResponseHeader('WWW-Authenticate');
    if (responseAuthenticationHeader === null || typeof (responseAuthenticationHeader) === 'undefined') {
      return false;
    }
    if (responseAuthenticationHeader.indexOf('Bearer') < 0) {
      return false;
    }
    return (__isForbidden(transport) || __isUnauthorized(transport));
  };

  var __isForbidden = function (transport) {
    return transport.status === 403;
  };

  var __isUnauthorized = function (transport) {
    return transport.status === 401;
  };

  var getResourceScope = function (transport) {
    if (typeof (transport) === 'undefined' || transport === null) {
      return null;
    }

    return WLAuthorizationManager.__getAuthorizationScope(transport.getResponseHeader('WWW-Authenticate'));
  };
  ...
  return {
    ...
    __invokeRegistrationRequest: __invokeRegistrationRequest,
    __invokeInstanceRegistrationRequest: __invokeInstanceRegistrationRequest,
    ...
    __isForbidden: __isForbidden,
    __isUnauthorized: __isUnauthorized,
    __cacheScopeByResource: __cacheScopeByResource,
    __getCachedAccessToken: __getCachedAccessToken,
    __removeTokenByScope: __removeTokenByScope,
